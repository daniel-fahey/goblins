...
;;        ;; let's replace it
;;        (when new-handler
;;          (unless (procedure? new-handler)
;;            (error 'become-failure "Tried to become a non-procedure handler: ~s"
;;                   new-handler))
...
;;                   [debug-name (object-name constructor)])
;;     (actormap-spawn!* actormap constructor kws kw-vals args debug-name))
